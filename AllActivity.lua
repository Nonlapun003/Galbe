...
--[[function buttonAddActivity(event)
    if(event.phase == "ended") then
    composer.gotoScene("Addactivity")
end
end]]
...
local function rowRender(event)
    ...
    if (row.index == 1) then
        rowTitle1 = display.newText(row, "รายการกิจกรรม                      เผาผลาญ(KCAL)",0, 0, "Quark-Bold.otf", fontSize)
...
myRectangle:setFillColor(135/255,206/255,235/255)
...
function createTableView(filter)
    ...
    c = 0
    for row in db:nrows("SELECT * FROM activity  WHERE type = 'Activity' and name LIKE \"%" .. sqlFilter.. "%\" ORDER BY name;") do
    c = c + 1
    table.insert(id,c)
...
function scene:show(event)
	...
	if (phase == "will") then
        ...
        db = sqlite.open(path)
        display.setDefault("background",230/255,255/255,255/255)
...
buttonFood = widget.newButton( {
            ...
            height= 35,
            shape = "Rectangle",
            label = "กิจกรรมในวันนี้",
            id = "button1",
            labelColor = { default={0, 0, 0, 1}, over={1, 1, 1, 1} },
            fillColor = { default={135/255,206/255,235/255, 1}, over={255/255, 127/255, 80/255, 1} },
            onEvent = clickButton
        }
    )
...
TextField = native.newTextField(cx-25,70,260,40)
